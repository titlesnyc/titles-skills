...
Get the target look/change first if it isn't clear. No dedicated tool for this one — it's the video-to-video operator, `vid2VidNode`, via the generic path: match a model with `titles_search_models({ operator: "vid2VidNode" })` (pick the best fit for the look they want — some are artist-trained, credit the creator), `titles_get_operator({ operator_id: "vid2VidNode" })` for the live input shape, then `titles_run_execution` with the chosen `model_id` and the change as the prompt. Video-to-video quotes for approval before anything runs.
